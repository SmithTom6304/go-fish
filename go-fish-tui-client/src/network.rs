...
mod tests {
    ...
    // ── Property 1: ClientMessage serialisation round-trip ────────────────────

    // Feature: go-fish-tui-client, Property 1: ClientMessage serialisation round-trip
    ...
    // ── Property 2: ServerMessage deserialisation round-trip ─────────────────

    // Feature: go-fish-tui-client, Property 2: ServerMessage deserialisation round-trip
    ...
    // ── Property 4: Identity message is the first ClientMessage sent ──────────

    // Feature: go-fish-tui-client, Property 4: Identity message is the first ClientMessage sent
...
/// Network events produced by the Network_Task and consumed by the Event_Loop.
#[derive(Debug, Clone)]
...
/// Runs the network task: reads WebSocket frames and forwards them as NetworkEvents,
/// and writes ClientMessages from the Event_Loop as JSON text frames.
pub async fn run_network_task(
    ...
    loop {
        tokio::select! {
            // Inbound: read a frame from the WebSocket
            frame = ws.next() => {
                match frame {
                        ...
                        match serde_json::from_str::<ServerMessage>(&text) {
                            Ok(msg) => {
                                if network_event_tx.send(NetworkEvent::Message(msg)).await.is_err() {
                                    // Event_Loop dropped its receiver; exit cleanly
                                    return;
                                }
                            }
                            Err(e) => {
                    ...
                    // Binary, Ping, Pong — silently ignore
                    Some(Ok(_)) => {}
                }
            }
            // Outbound: receive a ClientMessage and send it as a JSON text frame
            msg = client_msg_rx.recv() => {
                match msg {
                    ...
                    None => {
                        // Event_Loop dropped its sender; exit cleanly
                        return;
                    }
                }
            }
