...
| `OpponentState` | `{ name, card_count, completed_book_count }` — opponents' visible state only |
| `GameSnapshot` | `{ hand_state, opponents, active_player, last_hook_outcome }` — full per-turn sync |
